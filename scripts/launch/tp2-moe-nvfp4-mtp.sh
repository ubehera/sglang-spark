#!/bin/bash
# Archetype recipe: hybrid-MoE + NVFP4 weights + trained MTP head, TP=2 cross-node,
# NEXTN k=3 speculative decoding, long context. For Qwen3.5/3.6-class hybrid-MoE
# NVFP4 checkpoints that ship a trained MTP (nextn) head.
#   Validated example:  MODEL_REPO=RedHatAI/Qwen3.6-35B-A3B-NVFP4
#
# GB10 unified-memory mitigations baked in: mem-frac 0.78, mamba extra_buffer +
# 0.7 mem ratio, cuda-graph-bs {1,2,4,8}, --disable-piecewise-cuda-graph (the
# sm_121 piecewise compiler hard-fails), stream-interval 32 (detokenizer-stall
# fix, upstream #22511), NEXTN k=3. Multimodal is intentionally OFF: NEXTN + an
# image input wedges flashinfer.prefill.plan() on this family.
#
# Usage:
#   MODEL_REPO=<hf-repo-id> ./tp2-moe-nvfp4-mtp.sh <node_rank>
#     head   (rank 0):  MODEL_REPO=RedHatAI/Qwen3.6-35B-A3B-NVFP4 ./tp2-moe-nvfp4-mtp.sh 0
#     worker (rank 1):  MODEL_REPO=RedHatAI/Qwen3.6-35B-A3B-NVFP4 ./tp2-moe-nvfp4-mtp.sh 1
set -euo pipefail
...
MODEL_REPO="${MODEL_REPO:?set MODEL_REPO (HuggingFace repo id), e.g. RedHatAI/Qwen3.6-35B-A3B-NVFP4}"
...
echo "[moe-nvfp4-mtp TP=2 NEXTN k=$NEXTN_STEPS rank=$NODE_RANK] $(date '+%Y-%m-%d %H:%M:%S') launching"
echo "  model: $MODEL_REPO  (path: $MODEL_PATH)"
echo "  num_steps=$NEXTN_STEPS num_draft_tokens=$NUM_DRAFT_TOKENS eagle_topk=$NEXTN_TOPK"
echo "  multimodal=OFF, cudagraphs=ON bs={1,2,4,8}, chunked-prefill=32K, mamba-mem=0.7, mem-frac=0.78, max-running=8, stream-interval=32"
