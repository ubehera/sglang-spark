...
5. **Recover:** the watcher now **auto-restarts** both ranks after stopping them (drop_caches → `RECLAIM_DELAY_S` reclaim wait → `systemctl start`, local+peer), loop-guarded by `AUTO_RESTART_MAX` per `AUTO_RESTART_WINDOW_S`. So a transient wedge self-recovers in ~one cold-load cycle. If `triggers.log` shows a `SUPPRESSED` row, the loop-guard tripped (too many restarts in the window) and the unit is intentionally left DOWN — fix the root cause, then `systemctl start` manually. Auto-restart history is in `/var/lib/sglang-watcher/autorestart.log`.
