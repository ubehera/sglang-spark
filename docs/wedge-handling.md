...
- **NVRM gating:** NVRM-OOM also fires during a healthy cold start (KV pool alloc, cudagraph capture, NEXTN draft load), so it is gated for the first ~180 s after the unit goes `active`. Detokenizer / hung_task / memcg signals fire immediately — by their nature they cannot false-positive during init.
